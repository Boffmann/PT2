...
void removeNonDirectFlights(std::map<int, AirportInfo>& airportInfo)
{
	...
	{
		std::remove_if(airport.second.m_routes.begin(), airport.second.m_routes.end(),[](std::pair<int, int> &routes){return routes.second >=1;});
	}
	

}

// ToDo 4.1b - For each route in AirportInfo::m_routes, calculate the distance between start and destination. 
//Store the results in AirportInfo::m_routeLengths. Use std::transform() and calculateDistanceBetween().
void calculateDistancePerRoute(std::map<int, AirportInfo>& airportInfo)
{
	//std::transform(airportInfo.begin(), airportInfo.end(), )
...
//Store the results in AirportInfo::m_averageRouteLength. Use std::accumulate().
